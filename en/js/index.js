...
// 2. Toast 功能 (取代 alert & welcomeAnimation)
...
if (hasBadWords(text)) return showToast("⚠️ Forbidden words in comment！", "danger");
...
let userData = { name: "User", avatar: "https://cdn-icons-png.flaticon.com/512/1144/1144760.png", uid: "anonymous" };
...
if (currentUser) {
    ...
    if (doc.exists) {
        ...
        userData.avatar = data.avatar || "images/defult-avatar.png";

    } else {

        userData.name = currentUser.displayName || "User";

        userData.avatar = currentUser.photoURL || "images/defult-avatar.png";
...
try {
    ...
    showToast("Error during posting. ", "danger");
            ...
            <div class="flex-grow-1 border-bottom pb-3">
                ...
                ${canManage ? `
                    ...
                    <div class="mt-2 small">
                        ...
                        <span class="text-primary cursor-pointer me-2" onclick="editComment('${id}')">Edit</span>

                        <span class="text-danger cursor-pointer" onclick="deleteComment('${id}')">Delete</span>
...
try {
    ...
    showToast("Error during deleteing.", "danger");
...
if (!text) return showToast("Blank comment！", "danger");
...
try {
    ...
    showToast("Error during reading profile!", "danger");
...
try {
    ...
    showToast("Failed during saving!", "danger");
...
if (mode === "login") {
    ...
    title.textContent = "Email Sign Up";
...
if (currentUser) {
    ...
    document.getElementById("userName").textContent = currentUser.displayName || "New Friend";
...
fileInput.onchange = async () => {
    ...
    if (!file || file.size > 5 * 1024 * 1024) return showToast("Only accepts images under 5MB.", "danger");
...
try {
    ...
    } else if (modalTitle.includes("Sign Up")) {
    ...
    } else if (modalTitle.includes("reset")) {

        await auth.sendPasswordResetEmail(email);

        showToast("Mailed letter to reset password!  ✉️");
