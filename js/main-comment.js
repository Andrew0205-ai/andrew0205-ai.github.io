...
async function loadComments(reset=false){
    ...
    snap.forEach(doc=>{
        ...
        <div class="d-flex mb-4" id="comment-${id}" data-uid="${d.uid}" aria-label="留言: ${d.text}">
            <img src="${d.avatar}" width="50" height="50" class="rounded-circle me-3 border shadow-sm" alt="${d.name} 的頭像">
            <div class="flex-grow-1 border-bottom pb-3">
                <div class="d-flex justify-content-between align-items-center">
                    <strong>${d.name} ${d.uid===ADMIN_UID?'<span class="badge bg-danger badge-red">板主</span>':''}</strong>
                    <small class="text-dark">${d.timestamp?new Date(d.timestamp.toDate()).toLocaleString():'剛剛'}</small>
                </div>
                <div class="mt-2 text-dark">${safeHtml}</div>
                ${canManage?`
                <div class="mt-2 small">
                    <span role="button" tabindex="0" class="text-primary cursor-pointer me-2" onclick="editComment('${id}')" aria-label="編輯留言">編輯</span>
                    <span role="button" tabindex="0" class="text-danger cursor-pointer" onclick="deleteComment('${id}')" aria-label="刪除留言">刪除</span>
                </div>`:""}
            </div>
        </div>`;
...
async function uploadImage(){
    ...
    fileInput.onchange = async ()=>{
        ...
        try{
            ...
            const data=await res.json();
            const input=document.getElementById("commentInput");
            input.value+=`\n![圖片: ${file.name || "留言圖片"}](${data.secure_url})\n`;
